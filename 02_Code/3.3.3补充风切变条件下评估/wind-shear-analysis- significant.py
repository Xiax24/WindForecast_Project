...
def create_sample_size_table(df, output_dir):
    ...
    sample_size_df = pd.DataFrame({
        'Wind Shear Category': category_counts.index,
        'Sample Size': category_counts.values,
        'Percentage (%)': (category_counts.values / len(df) * 100).round(1)
    })
    
    # 按照逻辑顺序重新排序
    order = ['Low Shear', 'Moderate Shear', 'High Shear']
    sample_size_df = sample_size_df.set_index('Wind Shear Category').reindex(order).reset_index()
    
    # 保存数据量统计表
    sample_size_path = f"{output_dir}/windshear_sample_size.csv"
...
def create_visualizations(rmse_df, df, significance_results, output_dir):
    """
    创建可视化图表
    """
    print("\n创建可视化图表...")
    ...
    for idx, category in enumerate(categories):
        ...
        best_strategy = rmse_values.index[0]  # 最优策略（RMSE最小）
        
        # 创建条形图
        bars = axes[idx].bar(range(len(rmse_values)), rmse_values.values, 
                            color=colors[idx], alpha=0.7)
        
        # 高亮最优策略
        bars[0].set_color(colors[idx])
        ...
        if category in significance_results:
            sig_tests = significance_results[category]
            
            # 找出与最优策略有显著差异的策略
            significant_pairs = []
            for test_pair, result in sig_tests.items():
                if result['significant'] and best_strategy in test_pair:
                    # 提取另一个策略名称
                    strategies = test_pair.split(' vs ')
                    other_strategy = strategies[1] if strategies[0] == best_strategy else strategies[0]
                    significant_pairs.append(other_strategy)
            
            # 在显著差异的策略上添加标记
            for i, strategy in enumerate(rmse_values.index):
                if strategy in significant_pairs:
                    # 添加显著性星号
                    axes[idx].text(i, rmse_values.iloc[i] + 0.05, '*', 
                                 ha='center', va='bottom', fontsize=16, 
                                 color='red', fontweight='bold')
        
        # 添加图例说明（只在第一个子图添加）
        if idx == 0:
            axes[idx].text(0.02, 0.98, '* Significantly different from best\n(p < 0.05)', 
                          transform=axes[idx].transAxes, fontsize=10, 
                          verticalalignment='top', bbox=dict(boxstyle='round', 
                          facecolor='wheat', alpha=0.8))
    
    plt.tight_layout()
    plt.savefig(f"{output_dir}/rmse_comparison_bars_with_significance.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    try:
        ...
        # 7. 创建可视化
        create_visualizations(rmse_df, df, significance_results, output_dir)
