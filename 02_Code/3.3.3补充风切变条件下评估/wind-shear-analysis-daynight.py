...
def create_visualizations(rmse_df, df, output_dir):
    ...
    colors = ['lightcoral', 'darkred', 'skyblue', 'darkblue', 'lightgreen', 'darkgreen']
    
    axes = axes.flatten()
    
    for idx, category in enumerate(categories):
        if category in rmse_df.columns and not rmse_df[category].isna().all():
            rmse_values = rmse_df[category].dropna().sort_values()
            ...
            if len(rmse_values) > 0:
                ...
                axes[idx].set_title(f'RMSE under {category}', fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/rmse_comparison_bars_6categories.png", dpi=300, bbox_inches='tight')
    plt.show()
