...
【R2 Point 18 的替代版本不在本文件】
审稿人建议把 Panel A 改成 P_modeled/P_observed 的分布图。该实现放在
figure-3-b_fixed.py 的 create_pdf_figures() 里 —— 那个文件已经持有相同的
80/20 划分与三个已训练模型，Panel A（总体）与 Panel B（分扇区）由同一批
模型出图才能保证两个面板严格同口径。输出：
    final_3_a_pdf_{ratio,nerr}_*.png    <- 本面板的替代
    final_3_b_pdf_{ratio,nerr}_*.png
本文件保留原散点/KDE 版本，未改动。
"""
