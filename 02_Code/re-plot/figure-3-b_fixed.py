...
# 铭牌装机容量。正文 2.1：134 台 x 1.5 MW = 201 MW。
# 不要改用"观测到的最大功率"（本记录是 193.5 MW）：实际出力几乎达不到铭牌值
# （检修、限电），拿它当分母会让归一化误差系统性偏大约 3.9%，而且与正文自己
# 写的 201 MW 矛盾。P_rated 是场站的铭牌属性，不是数据的统计量。
RATED_POWER_MW = 201.0
...
# 图例用缩写，与 inset 的 HH/SR/ER 以及正文口径一致。
# 全称只在图注里定义一次（正文现有图注即如此），图内不重复。
SHORT_NAMES = {
    'Hub-height': 'HH',
    'Standard REWS': 'SR',
    'Extended REWS': 'ER',
...
class ScatterOverlayVisualizer:
    ...
    def _rated_power(self):
        """铭牌装机容量（常数，见 RATED_POWER_MW）。

        顺带核对一下观测最大功率没有超过铭牌值——超了说明功率列的单位
        或量纲有问题，那时归一化的百分比就不可信了。
        """
        obs_max = float(max(self.train_df['power'].max(),
                            self.test_df['power'].max()))
        if obs_max > RATED_POWER_MW:
            raise ValueError(
                f"观测最大功率 {obs_max:.1f} MW 超过铭牌装机 "
                f"{RATED_POWER_MW:.1f} MW —— 请先核对 power 列的单位。")
        print(f"  [P_rated] 铭牌 {RATED_POWER_MW:.1f} MW"
              f"（观测最大 {obs_max:.1f} MW = 铭牌的 {obs_max/RATED_POWER_MW:.1%}）")
    ...
    def report_ratio_tail(self):
        ...
        print("=" * 74)
        print(f"  铭牌装机 P_rated = {rated:.1f} MW，测试集 N = {len(y_true)}")
        ...
        for ax, (title, df_sub, sector_key) in zip(axes, panels):
            ...
            for model_name in ['Hub-height', 'Standard REWS', 'Extended REWS']:
                ...
                ax.plot(grid, pdf, color=c, linewidth=PDF_LINEWIDTH,
                        label=SHORT_NAMES[model_name], zorder=3)
                if fill:
