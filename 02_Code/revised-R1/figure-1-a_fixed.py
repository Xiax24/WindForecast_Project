...
def plot_windrose_custom_legend(csv_file, ws_col, wd_col, output_path, 
    ...
    import sys
    sys.path.append('/Users/xiaxin/work/WindForecast_Project/02_Code/re-plot')  # qc_common所在目录
    from qc_common import qc_power_only

    data = pd.read_csv(csv_file)
    data = qc_power_only(data)
    from qc_common import qc_power_only, report_sectors

    data = pd.read_csv(csv_file)
    data = qc_power_only(data)
    report_sectors(data, tag='windrose base')
    ...
    print(f"power缺测: {raw['power'].isna().sum():,}")
    print(f"power负值: {(raw['power'] < 0).sum():,}")
    print(f"✓ 数据: {len(ws):,} 点")
