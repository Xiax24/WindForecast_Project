...
class FourStrategyComparison:
    ...
    def compare_all_strategies(self):
        ...
        # 进行显著性检验（分扇区，供 Figure 4B/4D 与 Text S5/S7 使用）
        self.perform_sector_significance_tests(results)
    ...
    def perform_sector_significance_tests(self, results):
        """分扇区 RMSE 与扇区内 Diebold-Mariano 检验（Figure 4B/4D 与 Text S5/S7）"""
        print("\n" + "="*70)
        print("分扇区 RMSE 与 Diebold-Mariano 检验")
        print("="*70)
        ...
        strategy_order = ['HH', 'SR', 'ER', 'ER_CAP', 'WDA']
        comparisons = [
            ('HH', 'SR'),
            ('SR', 'ER'),
            ('ER', 'WDA'),
        ...
        for sec, mask in sector_masks.items():
            n_sec = int(mask.sum())
            y_t = results['HH']['y_true'][mask]

            print(f"\n--- 扇区: {sec}  (N = {n_sec}) ---")
            print(f"{'策略':<20} {'RMSE (MW)':>12} {'R²':>10} {'Corr':>10}")
            print("-"*54)
            ...
            for s in strategy_order:
                if s not in results:
                    continue
                y_p = results[s]['y_pred'][mask]
                rmse_sec[s] = float(np.sqrt(mean_squared_error(y_t, y_p)))
                r2_sec = r2_score(y_t, y_p)
                corr_sec = float(np.corrcoef(y_t, y_p)[0, 1])
                label = 'ER(300 trees)' if s == 'ER_CAP' else s
                print(f"{label:<20} {rmse_sec[s]:>12.2f} {r2_sec:>10.4f} {corr_sec:>10.4f}")

            print(f"\n  扇区内 DM 检验:")
            for s1, s2 in comparisons:
                if s1 not in results or s2 not in results:
                    continue
                errors1 = y_t - results[s1]['y_pred'][mask]
                errors2 = y_t - results[s2]['y_pred'][mask]
                dm_stat, p_val = diebold_mariano_test(errors1, errors2)
                ...
                improve_pct = (rmse_sec[s1] - rmse_sec[s2]) / rmse_sec[s1] * 100
                print(f"    {s2} vs {s1}: DM = {dm_stat:>7.4f}, "
                      f"p = {p_val:.4f} {sig_marker}, RMSE改进 = {improve_pct:>6.2f}%")

        print("\n显著性标记: *** p<0.01, ** p<0.05, * p<0.1, n.s. 不显著")
        print("注: DM 统计量为正表示后者优于前者；柱状图星号仅在优于 HH 且显著时标注")
        print("="*70)
    ...
    def create_visualization(self, results):
        ...
        # ========================================================================
        # 右边：横向柱状图 - Free-stream 和 Wake 扇区的 RMSE 对比
        # ========================================================================
        ...
        rmse_by_sector = {'free': [], 'wake': []}
        stars_by_sector = {'free': [], 'wake': []}

        for sec, mask in sector_masks.items():
            y_t = results['HH']['y_true'][mask]
            err_hh = y_t - results['HH']['y_pred'][mask]
            rmse_hh = float(np.sqrt(mean_squared_error(y_t, results['HH']['y_pred'][mask])))

            for strategy_name in strategy_names:
                y_p = results[strategy_name]['y_pred'][mask]
                r = float(np.sqrt(mean_squared_error(y_t, y_p)))
                rmse_by_sector[sec].append(r)

                if strategy_name == 'HH':
                    stars_by_sector[sec].append('')
                    continue

                _, p_val = diebold_mariano_test(err_hh, y_t - y_p)
                # 仅当优于 HH 且显著时打星，避免"显著更差"被误标为改进
                if r < rmse_hh and p_val < 0.01:
                    stars_by_sector[sec].append('***')
                elif r < rmse_hh and p_val < 0.05:
                    stars_by_sector[sec].append('**')
                elif r < rmse_hh and p_val < 0.1:
                    stars_by_sector[sec].append('*')
                else:
                    stars_by_sector[sec].append('')

        # x 轴范围（如需从 0 起不截断，把 bar_xmin 改为 0.0）
        # _all_rmse = [v for v in rmse_by_sector['free'] + rmse_by_sector['wake']
        #              if not np.isnan(v)]
        # bar_xmin = float(np.floor(min(_all_rmse) - 2))
        # bar_xmax = float(np.ceil(max(_all_rmse) + 2))
        # x 轴范围（手动设定；设为 None 则自动按数据取值）
        bar_xmin = 30.0
        bar_xmax = 45.0
        bar_xtick_interval = 5.0   # 刻度间隔，设为 None 则用 matplotlib 默认

        if bar_xmin is None or bar_xmax is None:
            _all_rmse = [v for v in rmse_by_sector['free'] + rmse_by_sector['wake']
                         if not np.isnan(v)]
            bar_xmin = float(np.floor(min(_all_rmse) - 2))
            bar_xmax = float(np.ceil(max(_all_rmse) + 2))
        ...
        def _draw_rmse_bar(ax, vals, stars, panel_label, show_xlabel):
            bars = ax.barh(y_pos, vals, color=colors, alpha=0.8, height=0.6)

            ax.text(0.99, 0.99, panel_label, transform=ax.transAxes,
                    fontsize=bar_label_fontsize, fontweight='bold',
                    verticalalignment='top', horizontalalignment='right')

            ax.set_yticks(y_pos)
            ax.set_yticklabels(strategy_names, fontsize=bar_label_fontsize)
            ax.tick_params(axis='x', labelsize=bar_label_fontsize)
            ax.set_xlim([bar_xmin, bar_xmax])
            # ax.set_xlim([bar_xmin, bar_xmax])
            if bar_xtick_interval is not None:
                ax.set_xticks(np.arange(bar_xmin, bar_xmax + 1e-9, bar_xtick_interval))
            ax.grid(True, axis='x', alpha=0.3)
            ax.invert_yaxis()

            if show_xlabel:
                ax.set_xlabel('RMSE (MW)', fontsize=bar_label_fontsize)

            for bar, val, st in zip(bars, vals, stars):
                if not np.isnan(val):
                    ax.text(val + (bar_xmax - bar_xmin) * 0.02,
                            bar.get_y() + bar.get_height() / 2,
                            f'{val:.2f}{st}', ha='left', va='center',
                            fontsize=bar_label_fontsize)

        ax_bar_free = fig.add_subplot(gs[0, 2])
        _draw_rmse_bar(ax_bar_free, rmse_by_sector['free'], stars_by_sector['free'],
                       'Free-stream', show_xlabel=False)

        ax_bar_wake = fig.add_subplot(gs[1, 2])
        _draw_rmse_bar(ax_bar_wake, rmse_by_sector['wake'], stars_by_sector['wake'],
                       'Wake-affected', show_xlabel=True)
