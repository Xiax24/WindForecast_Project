...
class ConsistentEEMDAnalyzer:
    """
    EEMD Analyzer with forced consistency in IMF counts
    """
        ...
        for var_name, result in self.decomposition_results.items():
            ...
            for i, imf in enumerate(IMFs):
                ...
                if zero_crossings > 0:
                    ...
                    char['Dominant_Period_Hours'] = dominant_period_points * 0.25
                else:
    ...
    def run_complete_analysis(self, target_imfs=12, ensemble_size=50, noise_std=0.2):
        """Run complete analysis with forced consistency"""
        ...
        for var in self.characteristics_df['Variable'].unique():
            ...
            summary = {
                ...
                'High_Freq_Energy': var_data[var_data['IMF'] <= 3]['Energy_Ratio'].sum(),
                'Medium_Freq_Energy': var_data[(var_data['IMF'] > 3) & (var_data['IMF'] <= 9)]['Energy_Ratio'].sum(),
                'Low_Freq_Energy': var_data[var_data['IMF'] > 9]['Energy_Ratio'].sum(),
...
if __name__ == "__main__":
    ...
    # Create analyzer
    analyzer = ConsistentEEMDAnalyzer(data_path, results_path)
    
    # Run analysis with forced consistency
    ...
    if results:
        print("\n" + "="*80)
        print("SUCCESS: All variables now have exactly 12 IMFs")
