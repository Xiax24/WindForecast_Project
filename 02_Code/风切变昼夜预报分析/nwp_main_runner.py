    ...
    try:
        ...
        for var_name, var_info in key_variables.items():
            ...
            if classes:
                ...
                # 1. 相关系数对比
                bars1 = axes[0, 0].bar(x - width/2, ec_corr, width, label='EC', color='#46C7FF', alpha=0.8)
                bars2 = axes[0, 0].bar(x + width/2, gfs_corr, width, label='GFS', color='#D93979', alpha=0.8)
                ...
                # 2. 偏差对比
                bars1 = axes[0, 1].bar(x - width/2, ec_bias, width, label='EC', color='#46C7FF', alpha=0.8)
                bars2 = axes[0, 1].bar(x + width/2, gfs_bias, width, label='GFS', color='#D93979', alpha=0.8)
                ...
                # 3. RMSE对比
                bars1 = axes[1, 0].bar(x - width/2, ec_rmse, width, label='EC', color='#46C7FF', alpha=0.8)
                bars2 = axes[1, 0].bar(x + width/2, gfs_rmse, width, label='GFS', color='#D93979', alpha=0.8)
                ...
                axes[1, 1].scatter(gfs_corr, gfs_rmse, s=120, alpha=0.8, color='#D93979', label='GFS', edgecolors='white', linewidth=1)
                ...
                for i, cls in enumerate(classes):
                    ...
                    axes[1, 1].annotate(f'GFS_{cls.replace(chr(10), "_")}', (gfs_corr[i], gfs_rmse[i]), 
                                       xytext=(-5, -5), textcoords='offset points', fontsize=7, color="#D93979")
