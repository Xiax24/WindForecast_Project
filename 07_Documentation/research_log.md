...
## 2025年5月26日 - 可以加在文章中的一个部分 - 这个是功率预测了。筛选特征吧
   计算所有观测数据对应prediction出整场power的能力，使用explainable model给出贡献，是否完全满足叶素动量理论，其他关键因子，
   四组对比试验：理论计算 ｜ hub-height wind | all observations | 延伸计算参数 四组对比试验
   使用回归方差解释给出气象影响与人为停机影响的差异。不同气象条件下的解释方差是否存在变化。
